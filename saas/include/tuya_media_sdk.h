 ...
 * @param channel   通道号
 */
typedef void (*audio_call_back)(const BYTE_T* buf, int len, int rate, int databits, int channel);
...
/**
 * 推送媒体流，请先运行ty_start_media
 * @param MediaType   媒体流类型
 * @param buf         媒体缓存
 * @param buflen      媒体缓存大小
 * @return            true:成功;false:失败
 */
bool ty_push_media(MediaType type, const unsigned char* buf, int buflen);
